    ...
    def get(self, request):
        with connection.cursor() as db_cursor:
            ...
            SELECT
                ...
                ga.title,
                ga.maker,
                ga.number_of_players,
                ga.skill_level,
                ga.game_type_id,
                gr.id gamer_id,
                u.first_name || ' ' || u.last_name AS full_name
            FROM levelupapi_game ga
            JOIN levelupapi_gamer gr
                ON gr.id = ga.gamer_id
            JOIN auth_user u
                ON u.id = gr.user_id
            """)
            ...
            for row in dataset:
                ...
                game = Game(row['id'], row['maker'],
                    row['number_of_players'], row['title'],
                    row['game_type_id'],row['skill_level'])
